...
export function getAllPosts() {
  const extractChildren = (dir) => {
    return (dir.children || []).map(file => {
      if (file.type === 'directory') {
        return extractChildren(file)
      }
      return file
    })
  }
...
//           return [
//             { params: { slug: [folder] } },
//             { params: { slug: [folder, 'index'] } },
//           ]
//         }
//         return {
//           params: {
//             slug: [folder, fileName.replace(/\.md$/, '')].filter(name => name !== '')
//           }
//         }
//       }
//     }).flat()
//   }
//   const routes = mapAllChildren('')
//   // console.log({ routes })
//   return routes.concat({ params: { slug: [''] } })
// }
...
export async function getPostData(slug) {
  ...
  if (fs.existsSync(filepath)) {
    ...
    if (fs.existsSync(indexFilePath)) {
      // index file existes!
      return {
        ...await renderMarkdown(indexFilePath + ".md"),
